...
- **Transparency**: Game operates normally with runtime services injected seamlessly
...
- **Characteristics**:
  ...
  - Designed for maximum compatibility across compilers
...
The framework uses a somewhat complex build system to create the single-header distribution:

1. **Modular Development**: API split into logical modules (`wolf_core.hpp`, `wolf_memory.hpp`, etc.)
2. **Dependency Analysis**: Build system analyzes `#include` dependencies between modules
3. **Automatic Ordering**: Headers are automatically ordered based on dependency analysis
4. **Single Header Generation**: All headers are flattened into `wolf_framework.hpp` with includes stripped
5. **Version Integration**: Generated version information is embedded during the build process
...
The framework intends to support all of the "Big 3" compilers:
- **MSVC**: Micro$oft
...
A complex build system manages the single-header generation:

1. **Auto-Discovery**: Automatically finds all `wolf_*.hpp` files
2. **Dependency Resolution**: Analyzes `#include` relationships
3. **Intelligent Ordering**: Determines correct header ordering
4. **Include Stripping**: Removes all includes in final output
5. **Version Injection**: Embeds build-time version information
...
The framework maintains ABI stability through:
...
### Minimal Overhead
- **Direct Function Calls**: Runtime API uses direct function pointers
- **Efficient Callbacks**: Callback systems designed for minimal performance impact
- **Selective Monitoring**: Memory watching and bitfield monitoring only active when requested

### Scalable Architecture
- **Dynamic Loading**: Mods loaded on-demand on launch
