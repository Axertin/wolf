    ...
    if (ImGui::Begin("DevTools", &showDevToolsWindow))
    {
        ...
        try
        {
            ...
            if (globalFlags && !globalGameStateDesc.empty())
            {
                ...
                {
                    for (unsigned i = 0; i < 86 && i < globalGameStateDesc.size(); i++)
                    {
                        if (globalGameStateDesc.contains(i))
                        {
                            checkboxBitField(globalGameStateDesc.at(i).c_str(), i, *globalFlags);
                        }
                        else
                        {
                            checkboxBitField("Unknown", i, *globalFlags);
                        }
                    }
                }
            }
        ...
        GROUP("World State")
        {
            ...
            if (ammyCollections)
            {
                ...
                GROUP("Animals Fed")
                {
                    for (unsigned i = 0; i < okami::Animals::NUM_ANIMALS && i < 32; i++)
                    {
                        drawStat(okami::Animals::GetName(i), ImGuiDataType_U16, &ammyCollections->world.numAnimalsFed[i]);
                    }
                }
            ...
            if (usableBrushes && obtainedBrushes)
            {
                ...
                if (brushUpgrades)
                {
                    GROUP("Brush Upgrades")
                    {
                        ...
                        {
                            std::string name = "BrushUpg" + std::to_string(i);
                            drawStat(name.c_str(), ImGuiDataType_U8, &(*brushUpgrades)[i]);
                        }
                    }
                }
            }
        ...
        // Game state monitoring status
        ImGui::Separator();
        ImGui::Text("Game State Monitoring: Active");
    }
    ImGui::End();
...
{
    wolf::logInfo("DevTools: Initializing GUI system");
    ...
    wolf::logInfo("DevTools: GUI initialization complete");
}
