...
static void logBitFieldChange(const char *fieldName, unsigned int bitIndex, bool oldValue, bool newValue,
                              const std::unordered_map<unsigned, std::string> &documentation)
{
    if (!documentation.contains(bitIndex))
    {
        wolf::logWarning("[Undocumented] BitField %s index %u was changed from %d to %d", fieldName, bitIndex, oldValue, newValue);
    }
    else
    {
        wolf::logInfo("BitField %s index %u (%s) was changed from %d to %d", fieldName, bitIndex, documentation.at(bitIndex).c_str(), oldValue, newValue);
    }
}
...
void initializeDevDataFinder()
{
    ...
    globalFlagsMonitor = wolf::createBitfieldMonitor(moduleBase + GLOBAL_GAME_STATE_FLAGS_OFFSET,
    ...
    for (int mapId = 0; mapId < okami::MapTypes::NUM_MAP_TYPES; mapId++)
    {
        ...
        auto monitor = wolf::createBitfieldMonitor(moduleBase + mapStateBitsOffset,
                                                   (256 + 7) / 8, // 256 bits converted to bytes
                                                   createMapStateCallback(mapId), ("Map " + std::to_string(mapId) + " WorldStateBits").c_str());
    ...
    wolf::logInfo("DevTools: Data finder initialization complete (%d monitors active)", 1 + static_cast<int>(mapStateMonitors.size()));
}
